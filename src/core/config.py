...
def check_resources_directory(path: str) -> None:
    logger.debug(f"Проверка наличия папки \"{path}\"")
    if not os.path.exists(path):
        logger.debug("Папка не обнаружена! Создаём папку...")
        os.mkdir(path)
        logger.warning(f"Папка \"{path}\" была автоматически создана.")
...
def get_config(
    ...
    check_resources_directory(directory_path)
...
__all__ = [
    "get_config",
    "DEFAULT_RESOURCES_DIRECTORY_PATH"
]
